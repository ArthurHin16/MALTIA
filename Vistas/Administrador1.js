...
import Empleado from '../components/cardEmpleado';
...
function Administrador1() {
    const [employees, setEmployees] = useState([]);
      ...
      useEffect(() => {
          axios.get("http://localhost:5000/consultants") //QUERY PARA VERIFICAR LOS ASESORES (TABLAS)
                .then((result) => {
                                    ...
                                    <main > {/*Componente CARD*/}
                                        <div class="row row-cols-md-4 text-center"> 
                                            {employees.map((employees) => (
                                                <Empleado //Mando la informacion obtenida a mi componente
